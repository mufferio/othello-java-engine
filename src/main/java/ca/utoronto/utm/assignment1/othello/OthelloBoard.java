...
public class OthelloBoard {
	...
	private char alternation(int row, int col, int drow, int dcol) {
        ...
        while (validCoordinate(r, c)) {
            ...
            if (curr != f) {
                int backr = row - drow;
                int backc = col - dcol;
                if (validCoordinate(backr, backc) && get(backr, backc) == EMPTY) {
                    return curr;
                }
                return EMPTY;
            }
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
        ...
        if (!validCoordinate(r, c) || get(r, c) != otherPlayer(player)) {
            return -1;
        }

        while (validCoordinate(r, c) && get(r, c) == otherPlayer(player)) {
            r += drow;
            c += dcol;
        }

        if (!validCoordinate(r, c) || get(r, c) != player) {
            return -1;
        }
	...
	private char hasMove(int row, int col, int drow, int dcol) {
        ...
        int rn = row + drow;
        int cn = col + dcol;
        if(!validCoordinate(rn, cn) || get(rn, cn) != otherPlayer(f)) {return EMPTY;}

        while (validCoordinate(rn, cn) && get(rn, cn) == otherPlayer(f)) {
            rn += drow;
            cn += dcol;
        }
        if (validCoordinate(rn, cn) && get(rn, cn) == EMPTY) {
            return f;
        }
        return EMPTY;
    }
            ...
            for (int col = 0; col < getDimension(); col++) {
                for (int drow = -1; drow <= 1; drow++) {
                    for (int dcol = -1; dcol <= 1; dcol++) {
                        ...
                        if (result == P1) p1Has = true;
                        if (result == P2) p2Has = true;
	...
	public boolean move(int row, int col, char player) {
        ...
        for (int drow = -1; drow <= 1; drow++) {
            for (int dcol = -1; dcol <= 1; dcol++) {
                ...
                int flipped = flip(row, col, drow, dcol, player);
                if (flipped > 0) {
                    valid = true;
                }
            }
    ...
    /**
     * a compy constructor for othelloboard to allow other player strategies
     * without touching class
     */
